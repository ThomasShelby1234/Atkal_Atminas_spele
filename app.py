...
def top_data():
    try:
        ...
        top_5 = sorted(top_rezultati, key=lambda x:(x['kliski'], x['laiks']))[:5]
...
def pievienot_rezultatu():
    ...
    try:
        pievienot(dati)
        top_5 = sorted(get_top_results(), key=lambda x:(x['kliski'], x['laiks']))[:5]
        with open('results.json','w',encoding='utf-8') as fail:
